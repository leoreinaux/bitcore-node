...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.smartcashd.height === 150) {
            node.services.smartcashd.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.smartcashd.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('smartcashd/hashblock', function(data) {
        bus.unsubscribe('smartcashd/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.smartcashd.height;
      node.services.smartcashd.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.smartcashd.sendTransaction(tx.serialize(), function(err, hash) {
