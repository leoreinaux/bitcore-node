...
var BitcoinService = proxyquire('../../lib/services/smartcashd', {
  fs: {
...
describe('Bitcoin Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var smartcashd = new BitcoinService(baseConfig);
      should.exist(smartcashd);
    });
    it('will create an instance without `new`', function() {
      var smartcashd = BitcoinService(baseConfig);
      should.exist(smartcashd);
    });
    it('will init caches', function() {
      var smartcashd = new BitcoinService(baseConfig);
      should.exist(smartcashd.utxosCache);
      should.exist(smartcashd.txidsCache);
      should.exist(smartcashd.balanceCache);
      should.exist(smartcashd.summaryCache);
      should.exist(smartcashd.transactionDetailedCache);

      should.exist(smartcashd.transactionCache);
      should.exist(smartcashd.rawTransactionCache);
      should.exist(smartcashd.blockCache);
      should.exist(smartcashd.rawBlockCache);
      should.exist(smartcashd.blockHeaderCache);
      should.exist(smartcashd.zmqKnownTransactions);
      should.exist(smartcashd.zmqKnownBlocks);
      should.exist(smartcashd.lastTip);
      should.exist(smartcashd.lastTipTimeout);
    });
    it('will init clients', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.nodes.should.deep.equal([]);
      smartcashd.nodesIndex.should.equal(0);
      smartcashd.nodes.push({client: sinon.stub()});
      should.exist(smartcashd.client);
    });
    it('will set subscriptions', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: []
      });
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._initDefaults({transactionConcurrency: 10});
      smartcashd.transactionConcurrency.should.equal(10);
      smartcashd._initDefaults({});
      smartcashd.transactionConcurrency.should.equal(5);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var methods = smartcashd.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var events = smartcashd.getPublishEvents();
      should.exist(events);
      events.length.should.equal(3);
      events[0].name.should.equal('smartcashd/rawtransaction');
      events[0].scope.should.equal(smartcashd);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('smartcashd/hashblock');
      events[1].scope.should.equal(smartcashd);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('smartcashd/addresstxid');
      events[2].scope.should.equal(smartcashd);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.subscribe = sinon.stub();
      smartcashd.unsubscribe = sinon.stub();
      var events = smartcashd.getPublishEvents();

      events[0].subscribe('test');
      smartcashd.subscribe.args[0][0].should.equal('rawtransaction');
      smartcashd.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      smartcashd.unsubscribe.args[0][0].should.equal('rawtransaction');
      smartcashd.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      smartcashd.subscribe.args[1][0].should.equal('hashblock');
      smartcashd.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      smartcashd.unsubscribe.args[1][0].should.equal('hashblock');
      smartcashd.unsubscribe.args[1][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var emitter = {};
      smartcashd.subscribe('hashblock', emitter);
      smartcashd.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      smartcashd.subscribe('rawtransaction', emitter2);
      smartcashd.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      smartcashd.subscribe('hashblock', emitter1);
      smartcashd.subscribe('hashblock', emitter2);
      smartcashd.subscribe('hashblock', emitter3);
      smartcashd.subscribe('hashblock', emitter4);
      smartcashd.subscribe('hashblock', emitter5);
      smartcashd.subscriptions.hashblock.length.should.equal(5);

      smartcashd.unsubscribe('hashblock', emitter3);
      smartcashd.subscriptions.hashblock.length.should.equal(4);
      smartcashd.subscriptions.hashblock[0].should.equal(emitter1);
      smartcashd.subscriptions.hashblock[1].should.equal(emitter2);
      smartcashd.subscriptions.hashblock[2].should.equal(emitter4);
      smartcashd.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      smartcashd.subscriptions.hashblock= [emitter1];
      smartcashd.unsubscribe('hashblock', emitter3);
      smartcashd.subscriptions.hashblock.length.should.equal(1);
      smartcashd.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      smartcashd.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(smartcashd.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var emitter = new EventEmitter();
      smartcashd.subscribeAddress(emitter, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will handle multiple address subscribers', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      smartcashd.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      smartcashd.subscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      smartcashd.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
      smartcashd.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      smartcashd.unsubscribeAddress(emitter1);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      smartcashd.unsubscribeAddress(emitter1, ['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo']);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter2];
      smartcashd.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      smartcashd.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      smartcashd.unsubscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.not.exist(smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      smartcashd.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'] = [emitter1, emitter2];
      sinon.spy(smartcashd, 'unsubscribeAddressAll');
      smartcashd.unsubscribeAddress(emitter1);
      smartcashd.unsubscribeAddressAll.callCount.should.equal(1);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      smartcashd.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      smartcashd.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'] = [emitter1, emitter2];
      smartcashd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      smartcashd.subscriptions.address['3CMNFxN1oHBc4R1EpboAL5yzHGgE611Xou'] = [emitter1];
      smartcashd.unsubscribeAddress(emitter1);
      smartcashd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      smartcashd.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'].length.should.equal(1);
      smartcashd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(smartcashd.subscriptions.address['3CMNFxN1oHBc4R1EpboAL5yzHGgE611Xou']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var config = smartcashd._getDefaultConfig();
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a bitcoin.conf file', function() {
      var TestBitcoin = proxyquire('../../lib/services/smartcashd', {
        fs: {
          readFileSync: readFileSync,
      ...
      var smartcashd = new TestBitcoin(baseConfig);
      smartcashd.options.spawn.datadir = '/tmp/.bitcoin';
      var node = {};
      smartcashd._loadSpawnConfiguration(node);
      should.exist(smartcashd.spawn.config);
      smartcashd.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var smartcashd = new TestBitcoin(config);
      smartcashd.options.spawn.datadir = './data';
      var node = {};
      smartcashd._loadSpawnConfiguration(node);
      smartcashd.options.spawn.datadir.should.equal('/tmp/.bitcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      ...
      var smartcashd = new TestBitcoin(baseConfig);
      (function() {
        smartcashd._loadSpawnConfiguration({datadir: './test'});
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the bitcoin.conf file', function() {
      ...
      var node = {};
      smartcashd._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        smartcashd.transactionDetailedCache.set(keys[i], {});
        smartcashd.utxosCache.set(keys[i], {});
        smartcashd.txidsCache.set(keys[i], {});
        smartcashd.balanceCache.set(keys[i], {});
        smartcashd.summaryCache.set(keys[i], {});
      }
      smartcashd._resetCaches();
      should.equal(smartcashd.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(smartcashd.utxosCache.get(keys[0]), undefined);
      should.equal(smartcashd.txidsCache.get(keys[0]), undefined);
      should.equal(smartcashd.balanceCache.get(keys[0]), undefined);
      should.equal(smartcashd.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.tryAllInterval = 1;
      smartcashd.nodes.push({
        client: {
      ...
      smartcashd._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        smartcashd.nodes[0].client.getInfo.callCount.should.equal(1);
        smartcashd.nodes[1].client.getInfo.callCount.should.equal(1);
        smartcashd.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
    it('will start using the current node index (round-robin)', function(done) {
      ...
      smartcashd._tryAllClients(function(client, next) {
        ...
        smartcashd.nodes[1].client.getInfo.callCount.should.equal(1);
        smartcashd.nodes[2].client.getInfo.callCount.should.equal(1);
        smartcashd.nodesIndex.should.equal(2);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert bitcoind-rpc error object into JavaScript error', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var error = smartcashd._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      smartcashd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      smartcashd.nodes.push({
        client: {
      ...
      smartcashd._initChain(function() {
        log.info.callCount.should.equal(1);
        smartcashd.getRawBlock.callCount.should.equal(1);
        smartcashd.getRawBlock.args[0][0].should.equal('genesishash');
        smartcashd.height.should.equal(5000);
        smartcashd.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      smartcashd._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      smartcashd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      smartcashd._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    ...
    it('will get default rpc port for livenet', function() {
      ...
      var smartcashd = new BitcoinService(config);
      smartcashd._getDefaultConf().rpcport.should.equal(9678);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var smartcashd = new BitcoinService(config);
      smartcashd._getDefaultConf().rpcport.should.equal(19678);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var smartcashd = new BitcoinService(config);
      should.equal(smartcashd._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var smartcashd = new BitcoinService(config);
      smartcashd._getNetworkConfigPath().should.equal('testnet3/bitcoin.conf');
    });
    it('will get default rpc port for regtest', function() {
      bitcore.Networks.enableRegtest();
      var config = {
      ...
      var smartcashd = new BitcoinService(config);
      smartcashd._getNetworkConfigPath().should.equal('regtest/bitcoin.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.node.network = bitcore.Networks.testnet;
      smartcashd._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.node.network = bitcore.Networks.testnet;
      bitcore.Networks.enableRegtest();
      smartcashd._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.node.network = bitcore.Networks.livenet;
      bitcore.Networks.enableRegtest();
      should.equal(smartcashd._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      smartcashd._rapidProtectedUpdateTip = sinon.stub();
      smartcashd.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      smartcashd._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      smartcashd._rapidProtectedUpdateTip = sinon.stub();
      smartcashd._zmqBlockHandler(node, message);
      smartcashd._rapidProtectedUpdateTip.callCount.should.equal(1);
      smartcashd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      smartcashd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      smartcashd._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      smartcashd.subscriptions.hashblock.push(emitter);
      emitter.on('smartcashd/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      var callCount = 0;
      smartcashd._updateTip = function() {
        callCount++;
      ...
      var count = 0;
      function repeat() {
        smartcashd._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub();
      ...
      smartcashd._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      smartcashd.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      smartcashd.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      smartcashd.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub();
      smartcashd._resetCaches = sinon.stub();
      smartcashd.on('tip', function(height) {
        smartcashd._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        smartcashd.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      smartcashd.on('tip', function() {
        smartcashd.syncPercentage.callCount.should.equal(0);
        done();
      });
      smartcashd._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using bitcore.Transaction', function() {
      ...
      tx.sign(privkey);
      var addresses = smartcashd._getAddressesFromTransaction(tx);
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      smartcashd._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      smartcashd.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('smartcashd/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      smartcashd._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      smartcashd.subscriptions.rawtransaction.push(emitter);
      emitter.on('smartcashd/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      smartcashd._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      smartcashd.subscriptions.rawtransaction.push(emitter);
      emitter.on('smartcashd/rawtransaction', function() {
        done();
      });
      var node = {};
      smartcashd._zmqTransactionHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      smartcashd.on('tx', function(buffer) {
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._updateTip = sinon.stub();
      smartcashd._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      smartcashd.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      smartcashd._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        smartcashd._updateTip.callCount.should.equal(11);
        smartcashd._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      smartcashd._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        smartcashd._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      smartcashd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        smartcashd._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      smartcashd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        smartcashd._zmqBlockHandler.callCount.should.equal(0);
        smartcashd._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var smartcashd = new BitcoinService(baseConfig);
      var node = {};
      smartcashd._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      smartcashd._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      smartcashd._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      smartcashd._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      smartcashd.on('tip', function(height) {
        ...
        smartcashd.height.should.equal(100);
        done();
      });
      smartcashd._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestBitcoinService = proxyquire('../../lib/services/smartcashd', {
        fs: {
          readFile: readFile
        }
      });
      var smartcashd = new TestBitcoinService(baseConfig);
      smartcashd.spawnStopTime = 1;
      smartcashd._process = {};
      smartcashd._process.kill = sinon.stub();
      smartcashd._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
        smartcashd._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      smartcashd._process.kill = sinon.stub().throws(error2);
      smartcashd._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._loadSpawnConfiguration = sinon.stub();
      smartcashd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      smartcashd._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give error from stopSpawnedBitcoin', function() {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._loadSpawnConfiguration = sinon.stub();
      smartcashd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
      smartcashd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var smartcashd = new TestBitcoinService(config);
      smartcashd.spawn = {};
      smartcashd._loadSpawnConfiguration = sinon.stub();
      smartcashd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
      smartcashd.node.stopping = true;
      smartcashd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      smartcashd._loadSpawnConfiguration = sinon.stub();
      smartcashd.spawn = {};
      smartcashd.spawn.exec = 'testexec';
      smartcashd.spawn.configPath = 'testdir/bitcoin.conf';
      smartcashd.spawn.datadir = 'testdir';
      smartcashd.spawn.config = {};
      smartcashd.spawn.config.rpcport = 20001;
      smartcashd.spawn.config.rpcuser = 'bitcoin';
      smartcashd.spawn.config.rpcpassword = 'password';
      smartcashd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

      smartcashd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      smartcashd._initZmqSubSocket = sinon.stub();
      smartcashd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      smartcashd._checkReindex = sinon.stub().callsArgWith(1, null);
      smartcashd._spawnChildProcess(function(err, node) {
        ...
        smartcashd._loadTipFromNode.callCount.should.equal(1);
        smartcashd._initZmqSubSocket.callCount.should.equal(1);
        should.exist(smartcashd._initZmqSubSocket.args[0][0].client);
        smartcashd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        smartcashd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(smartcashd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn smartcashd spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/smartcashd', {
        fs: {
      ...
      smartcashd._loadSpawnConfiguration = sinon.stub();
      smartcashd.spawn = {};
      smartcashd.spawn.exec = 'smartcashd';
      smartcashd.spawn.datadir = '/tmp/bitcoin';
      smartcashd.spawn.configPath = '/tmp/bitcoin/bitcoin.conf';
      smartcashd.spawn.config = {};
      smartcashd.spawnRestartTime = 1;
      smartcashd._loadTipFromNode = sinon.stub().callsArg(1);
      smartcashd._initZmqSubSocket = sinon.stub();
      smartcashd._checkReindex = sinon.stub().callsArg(1);
      smartcashd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      smartcashd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
      sinon.spy(smartcashd, '_spawnChildProcess');
      smartcashd._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            smartcashd._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      smartcashd._spawnChildProcess(function(err) {
        ...
        smartcashd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        smartcashd.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn smartcashd spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/smartcashd', {
        fs: {
      ...
      smartcashd._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            smartcashd._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      var smartcashd = new TestBitcoinService(baseConfig);
      smartcashd.startRetryInterval = 1;
      smartcashd._loadSpawnConfiguration = sinon.stub();
      smartcashd.spawn = {};
      smartcashd.spawn.exec = 'testexec';
      smartcashd.spawn.configPath = 'testdir/bitcoin.conf';
      ...
      smartcashd.spawn.config.rpcpassword = 'password';
      smartcashd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      smartcashd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      smartcashd._spawnChildProcess(function(err) {
        smartcashd._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      smartcashd._initZmqSubSocket = sinon.stub();
      smartcashd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      smartcashd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var smartcashd = new BitcoinService(config);
      smartcashd.node.stopping = true;
      smartcashd.startRetryInterval = 100;
      smartcashd._loadTipFromNode = sinon.stub();
      smartcashd._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        smartcashd._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      smartcashd.startRetryInterval = 1;
      var config = {};
      smartcashd._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        smartcashd._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      smartcashd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      smartcashd._connectProcess(config, function(err, node) {
        should.not.exist(err);
        smartcashd._loadTipFromNode.callCount.should.equal(1);
        smartcashd._initZmqSubSocket.callCount.should.equal(1);
        smartcashd._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error from connectProcess', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      smartcashd.options = {
        connect: [
          {}
        ]
      ...
      smartcashd.start(function(err) {
        smartcashd._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      var node = {};
      smartcashd._initChain = sinon.stub().callsArg(0);
      smartcashd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      smartcashd.options = {
        spawn: {}
      };
      smartcashd.start(function(err) {
        should.not.exist(err);
        smartcashd.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      smartcashd._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      smartcashd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      smartcashd.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    it('will give error from syncPercentage', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      smartcashd.isSynced(function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      smartcashd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      smartcashd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      smartcashd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      smartcashd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      smartcashd.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var args = smartcashd._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var args = smartcashd._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      var options = {};
      smartcashd.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      var options = {};
      smartcashd.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        smartcashd.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      smartcashd.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      smartcashd.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = smartcashd._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = smartcashd._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = smartcashd._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      smartcashd._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        smartcashd._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      smartcashd.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      smartcashd.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      smartcashd.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      smartcashd.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        smartcashd.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
          ...
          smartcashd.getAddressTxids(address, {queryMempoolOnly: true}, function(err, txids) {
            if (err) {
              return done(err);
            }
            getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      tx.height = -1;
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.height = 10;
      var confirmations = smartcashd._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var result = {};
      smartcashd._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var result = {};
      smartcashd._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var result = {};
      smartcashd._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var smartcashd = new BitcoinService(baseConfig);
      var result = {};
      smartcashd._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = smartcashd._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      smartcashd.height = 300;
      var addresses = {};
      smartcashd._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      smartcashd._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      smartcashd._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from bitcore addresses', function() {
      ...
      var smartcashd = new BitcoinService(baseConfig);
      var strings = smartcashd._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var smartcashd = new BitcoinService(baseConfig);
      (function() {
        smartcashd._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      smartcashd.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      smartcashd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      smartcashd.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.maxAddressesQuery = 100;
      smartcashd.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      smartcashd.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      smartcashd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      smartcashd.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      smartcashd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      smartcashd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      smartcashd.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      smartcashd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      smartcashd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      smartcashd.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(smartcashd, '_paginateTxids');
      smartcashd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      smartcashd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      smartcashd.getAddressSummary(address, options, function(err, summary) {
        smartcashd._paginateTxids.callCount.should.equal(1);
        smartcashd._paginateTxids.args[0][1].should.equal(0);
        smartcashd._paginateTxids.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      smartcashd.getAddressSummary(address, options, function(err, summary) {
        ...
        smartcashd.getAddressTxids.callCount.should.equal(1);
        smartcashd.getAddressBalance.callCount.should.equal(1);
        smartcashd.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          smartcashd.getAddressTxids.callCount.should.equal(1);
          smartcashd.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      smartcashd.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginateTxids', function(done) {
      ...
      smartcashd._paginateTxids = sinon.stub().throws(new Error('test'));
      var address = '3NbU8XzUgKyuCgYgZEKsBtUvkTm2r7Xgwj';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      smartcashd.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      smartcashd.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      smartcashd.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      smartcashd.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      smartcashd.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as bitcore object from height', function(done) {
      ...
      smartcashd.getBlock(0, function(err, block) {
        should.not.exist(err);
    ...
    it('will getblock as bitcore object', function(done) {
      ...
      smartcashd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      smartcashd.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      smartcashd.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      smartcashd.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      smartcashd.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      smartcashd.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      smartcashd.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      smartcashd.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      smartcashd._maybeGetBlockHash('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      smartcashd._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      smartcashd._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      smartcashd.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      smartcashd.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      smartcashd.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    ...
    it('will give rpc error', function() {
      ...
      smartcashd.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      smartcashd.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      smartcashd.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = bitcore.Transaction();
      (function() {
        smartcashd.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      smartcashd.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      smartcashd.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      smartcashd.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      smartcashd.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      smartcashd.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      smartcashd.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.getInfo(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      smartcashd.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      smartcashd.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      smartcashd.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    it('will callback if spawn is not set', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.stop(done);
    });
    it('will exit spawned process', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.spawn = {};
      smartcashd.spawn.process = new EventEmitter();
      smartcashd.spawn.process.kill = sinon.stub();
      smartcashd.stop(done);
      smartcashd.spawn.process.kill.callCount.should.equal(1);
      smartcashd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      smartcashd.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      smartcashd.spawn.process.kill.callCount.should.equal(1);
      smartcashd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      smartcashd.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var smartcashd = new BitcoinService(baseConfig);
      smartcashd.shutdownTimeout = 300;
      smartcashd.spawn = {};
      smartcashd.spawn.process = new EventEmitter();
      smartcashd.spawn.process.kill = sinon.stub();
      smartcashd.stop(function(err) {
